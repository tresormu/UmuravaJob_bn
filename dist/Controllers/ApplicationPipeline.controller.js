...
import { sendApplicationReceivedEmail, sendShortlistedEmail, } from "../utils/email.js";
...
const normalizeHeader = (value) => String(value ?? "")
    .trim()
    .toLowerCase();
const coerceAnswerValue = (type, value, options) => {
...
class ApplicationPipelineController {
    static async createApplication(req, res) {
        try {
            ...
            const { fullName, email, phone, location, resumeUrl, resumeFileName, resumeText, linkedInUrl, portfolioUrl, answers, } = req.body;
            if (!fullName) {
            ...
            const invalidAnswer = providedAnswers.find((a) => !a.questionId || !questionById.has(a.questionId));
            if (invalidAnswer) {
    ...
    static async listJobApplications(req, res) {
        try {
            if (!req.user) {
    ...
    static async updateStatus(req, res) {
        try {
            ...
            const updated = await Application.findOneAndUpdate({ _id: id, recruiterId: req.user.id }, { $set: { status } }, { new: true, runValidators: true }).populate("candidateId");
            if (!updated) {
    ...
    static async uploadExcel(_req, res) {
        try {
            const req = _req;
            if (!req.user) {
                return res.status(401).json({ message: "Unauthorized" });
                ...
                try {
                    await session.withTransaction(async () => {
                        ...
                        if (!candidate) {
                            ...
                            addIfDefined(candidateData, "location", toOptionalTrimmedString(row.location ?? row.Location));
                            addIfDefined(candidateData, "resumeUrl", toOptionalTrimmedString(row.resumeUrl ?? row["resume url"]));
                            addIfDefined(candidateData, "resumeFileName", toOptionalTrimmedString(row.resumeFileName ?? row["resume file name"]));
                            addIfDefined(candidateData, "resumeText", toOptionalTrimmedString(row.resumeText ?? row["resume text"]));
                            addIfDefined(candidateData, "linkedInUrl", toOptionalTrimmedString(row.linkedInUrl ?? row["linkedin url"]));
                            addIfDefined(candidateData, "portfolioUrl", toOptionalTrimmedString(row.portfolioUrl ?? row["portfolio url"]));
                            const createdCandidates = await Candidate.create([candidateData], { session });
                            const createdCandidate = createdCandidates[0];
                            if (!createdCandidate) {
                                throw new Error("CANDIDATE_CREATE_FAILED");
...
//# sourceMappingURL=ApplicationPipeline.controller.js.map